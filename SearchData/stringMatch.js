...
function searchElements(){
	var searchThisElement = document.getElementById("inputField").value;
		function filterArrayElements(arrayFirst){
			var inputFirst =  searchThisElement.charAt();
				if(inputFirst == arrayFirst.charAt()){
					return arrayFirst.charAt();
				}
		}
		var filter = stringObject.filter(filterArrayElements);
		if(filter.length == 0){
